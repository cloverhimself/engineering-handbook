...
- Do not mark a phase complete until its required checks were actually run and relevant evidence exists.
...
- Distinguish automated tests/checks, manual verification, runtime/provider verification, and unverified behavior.
...
- Before final project completion, run supervisor mode and verify the canonical repository contains the reviewed state.
...
- [ ] automated vs manual vs provider/runtime verification is clear
...
### MVP
Real users require real correctness on core workflows, secure defaults, durable persistence/migrations, meaningful tests, basic observability, backups where needed, and cost-aware infrastructure.
